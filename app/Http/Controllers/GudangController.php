...
class GudangController extends Controller
{
    ...
    // PaketController.php
    public function daftarPaketKurir(Request $request)
    {
        $pakets = Input::where('kurir_id', $request->user()->id)
            ->whereIn('status', ['dalam_perjalanan', 'selesai'])
            ->get();

        return response()->json($pakets);
    }
    public function ajukanPindahGudang(Request $request)
    {
        $request->validate([
            'no_resi' => 'required|string|exists:inputs,no_resi',
            'gudang_tujuan' => 'required|string'
        ...
        $paket->update([
            'gudang_tujuan' => $request->gudang_tujuan,
            'status_pemindahan' => 'menunggu',
        ]);

        return response()->json(['success' => true, 'message' => 'Permintaan pindah dikirim']);
    }
    public function statusPermintaanPindah(Request $request)
    {
        $pakets = Input::whereNotNull('status_pemindahan')
